...
func TestGitHubPullRequest_Post(t *testing.T) {
	...
	// https://github.com/khulnasoft/codebase/pull/2
	owner := "khulnasoft"
	repo := "codebase"
	pr := 2
	...
	comment := &codebase.Comment{
		Result: &filter.FilteredDiagnostic{
			Diagnostic: &rdf.Diagnostic{
				Location: &rdf.Location{
					Path: "watchdogs.go",
				},
				Message: "[codebase] test",
			},
			InDiffFile:    true,
			InDiffContext: true,
		},
	}
	// https://github.com/khulnasoft/codebase/pull/2/files#diff-ed1d019a10f54464cfaeaf6a736b7d27L20
...
diff --git a/codebase.go b/codebase.go
		...
		case http.MethodGet:
			...
			default:
				cs := []*github.PullRequestComment{
					{
						Path:        github.String("codebase.go"),
						Line:        github.Int(2),
...
func TestGitHubPullRequest_Post_toomany(t *testing.T) {
	...
	var comments []*codebase.Comment
	for i := 0; i < 100; i++ {
		comments = append(comments, &codebase.Comment{
			Result: &filter.FilteredDiagnostic{
				Diagnostic: &rdf.Diagnostic{
					Location: &rdf.Location{
						Path: "codebase.go",
...
func TestGitHubPullRequest_workdir(t *testing.T) {
	...
	want := "a/b/c"
	g.Post(ctx, &codebase.Comment{Result: &filter.FilteredDiagnostic{
