...
import (
	...
	"github.com/khulnasoft/codebase/packages/go-actions-toolkit/core"
...
var _ codebase.CommentService = &GitHubActionLogWriter{}
...
func (lw *GitHubActionLogWriter) Post(_ context.Context, c *codebase.Comment) error {
	lw.reportNum++
...
func ReportAsGitHubActionsLog(toolName, defaultLevel string, d *rdf.Diagnostic) {
	mes := fmt.Sprintf("[%s] reported by codebase 🐶\n%s\n\nRaw Output:\n%s",
...
func warnTooManyAnnotation() {
	core.Error(`codebase: Too many results (annotations) in diff.
