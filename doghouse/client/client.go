...
const baseEndpoint = "https://codebase.khulnasoft.com"
...
type DogHouseClient struct {
	Client *http.Client
	// Base URL for API requests. Defaults is https://codebase.khulnasoft.com
	BaseURL *url.URL
}
...
func New(client *http.Client) *DogHouseClient {
	...
	base := baseEndpoint
	if baseEnvURL := os.Getenv("CODEBASE_GITHUB_APP_URL"); baseEnvURL != "" {
		base = baseEnvURL
	}
	var err error
...
func (c *DogHouseClient) Check(ctx context.Context, req *doghouse.CheckRequest) (*doghouse.CheckResponse, error) {
	...
	httpReq.Header.Set("User-Agent", fmt.Sprintf("codebase/%s", commands.Version))
