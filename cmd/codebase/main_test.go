...
`
	...
	beforef, err := os.CreateTemp("", "codebase-test")
	if err != nil {
		t.Fatal(err)
	}
	defer beforef.Close()
	defer os.Remove(beforef.Name())
	afterf, err := os.CreateTemp("", "codebase-test")
	if err != nil {
		t.Fatal(err)
	}
	defer afterf.Close()
...
func TestRun_project(t *testing.T) {
	...
	t.Run("invalid config", func(t *testing.T) {
		conffile, err := os.CreateTemp("", "codebase-test")
		if err != nil {
			t.Fatal(err)
		}
	...
	t.Run("conffile allows to be prefixed with '.' and '.yaml' file extension", func(t *testing.T) {
		for _, n := range []string{".codebase.yml", "codebase.yaml"} {
