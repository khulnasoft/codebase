...
import (
	...
	"github.com/khulnasoft/codebase/commands"
	"github.com/khulnasoft/codebase/filter"
	"github.com/khulnasoft/codebase/parser"
	"github.com/khulnasoft/codebase/project"
	bbservice "github.com/khulnasoft/codebase/service/bitbucket"
	gerritservice "github.com/khulnasoft/codebase/service/gerrit"
	githubservice "github.com/khulnasoft/codebase/service/github"
	"github.com/khulnasoft/codebase/service/github/githubutils"
	gitlabservice "github.com/khulnasoft/codebase/service/gitlab"
)

const usageMessage = "" +
	`Usage:	codebase [flags]
	codebase accepts any compiler or linter results from stdin and filters
	them by diff for review. codebase also can posts the results as a comment to
	GitHub if you use codebase in CI service.`
...
const (
	...
	efmsDoc       = `list of supported machine-readable format and errorformat (https://github.com/codebase/errorformat)`
...
`
	reporterDoc = `reporter of codebase results. (local, github-check, github-pr-check, github-pr-review, gitlab-mr-discussion, gitlab-mr-commit)
	"local" (default)
	...
	"github-check"
		...
		Option 1) Run codebase from GitHub Actions w/ secrets.GITHUB_TOKEN
			...
			Set CODEBASE_GITHUB_API_TOKEN with secrets.GITHUB_TOKEN. e.g.
		...
		Option 2) Install codebase GitHub Apps
			...
			You can get token from https://codebase.app/gh/<owner>/<repo-name>.
			$ export CODEBASE_TOKEN="xxxxx"

			Note: Token is not required if you run codebase in Travis CI.
	...
	"github-pr-review"
		...
		1. Set CODEBASE_GITHUB_API_TOKEN environment variable.
	...
	"gitlab-mr-discussion"
		...
		1. Set CODEBASE_GITLAB_API_TOKEN environment variable.
	...
	CODEBASE_INSECURE_SKIP_VERIFY to skip verifying SSL (please use this at your own risk)
		$ export CODEBASE_INSECURE_SKIP_VERIFY=true

	For non-local reporters, codebase automatically get necessary data from
	...
	you want (e.g. run codebase in Jenkins).

		$ export CI_PULL_REQUEST=14 # Pull Request number (e.g. 14)
		$ export CI_COMMIT="$(git rev-parse @)" # SHA1 for the current build
		$ export CI_REPO_OWNER="khulnasoft" # repository owner
		$ export CI_REPO_NAME="codebase" # repository name
`
...
func usage() {
	...
	fmt.Fprintln(os.Stderr, "See https://github.com/khulnasoft/codebase for more detail.")
	os.Exit(2)
}
...
func main() {
	...
	if err := run(os.Stdin, os.Stdout, opt); err != nil {
		fmt.Fprintf(os.Stderr, "codebase: %v\n", err)
		os.Exit(1)
	}
}
...
func run(r io.Reader, w io.Writer, opt *option) error {
	...
	var cs codebase.CommentService
	var ds codebase.DiffService
	...
	if isProject {
		...
		cs = codebase.NewUnifiedCommentWriter(w)
	} else {
		cs = codebase.NewRawCommentWriter(w)
	}
	...
	case "github-pr-review":
		...
		if !isPR {
			fmt.Fprintln(os.Stderr, "codebase: this is not PullRequest build.")
			return nil
		}
...
[2]: https://help.github.com/en/actions/automating-your-workflow-with-github-actions/development-tools-for-github-actions#logging-commands`)
			...
			cs = codebase.MultiCommentService(gs, cs)
		}
		ds = gs
	case "gitlab-mr-discussion":
		...
		cs = codebase.MultiCommentService(gc, cs)
	...
	case "bitbucket-code-report":
		...
		if !(opt.filterMode == filter.ModeDefault || opt.filterMode == filter.ModeNoFilter) {
			...
			// - All (50)
			log.Printf("codebase: [bitbucket-code-report] supports only with filter.ModeNoFilter for now")
		}
		opt.filterMode = filter.ModeNoFilter
		ds = &codebase.EmptyDiff{}
	case "local":
	...
	app := codebase.NewCodebase(toolName(opt), p, cs, ds, opt.filterMode, opt.failOnError)
...
func diffService(s string, strip int) (codebase.DiffService, error) {
	...
	d := codebase.NewDiffCmd(cmd, strip)
	return d, nil
}
...
func insecureSkipVerify() bool {
	return os.Getenv("CODEBASE_INSECURE_SKIP_VERIFY") == "true"
}

func githubService(ctx context.Context, opt *option) (gs *githubservice.PullRequest, isPR bool, err error) {
	token, err := nonEmptyEnv("CODEBASE_GITHUB_API_TOKEN")
	if err != nil {
		return nil, isPR, err
	}
...
func getPullRequestIDByBranchOrCommit(ctx context.Context, client *github.Client, info *cienv.BuildInfo) (int, error) {
	...
	if *pullRequests.Total == 0 {
		return 0, fmt.Errorf("codebase: PullRequest not found, query: %s", preparedQuery)
	}
...
func githubBaseURL() (*url.URL, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("GitHub base URL from codebase default is invalid: %v, %w", defaultGitHubAPI, err)
	}
	return u, nil
}

func gitlabBuildWithClient() (*cienv.BuildInfo, *gitlab.Client, error) {
	token, err := nonEmptyEnv("CODEBASE_GITLAB_API_TOKEN")
	if err != nil {
		return nil, nil, err
	}
...
func readConf(conf string) ([]byte, error) {
	...
	return nil, errors.New(".codebase.yml not found")
}
