...
func newDoghouseCli(ctx context.Context) (client.DogHouseClientInterface, error) {
	...
	skipDoghouseServer := (os.Getenv("CODEBASE_SKIP_DOGHOUSE") == "true" || cienv.IsInGitHubAction()) && os.Getenv("CODEBASE_TOKEN") == ""
...
func newDoghouseServerCli(ctx context.Context) *client.DogHouseClient {
	...
	if token := os.Getenv("CODEBASE_TOKEN"); token != "" {
...
func checkResultSet(ctx context.Context, r io.Reader, opt *option, isProject bool) (*codebase.ResultMap, error) {
	resultSet := new(codebase.ResultMap)
	if isProject {
		...
		resultSet.Store(toolName(opt), &codebase.Result{
			Level:       opt.level,
...
func postResultSet(ctx context.Context, resultSet *codebase.ResultMap,
	ghInfo *cienv.BuildInfo, cli client.DogHouseClientInterface, opt *option) (*codebase.FilteredResultMap, error) {
	...
	resultSet.Range(func(name string, result *codebase.Result) {
		...
		g.Go(func() error {
			...
			if res.ReportURL != "" {
				...
				filteredResultSet.Store(name, &codebase.FilteredResult{
					Level:              result.Level,
			...
			// codebase run status (#446) to merge PRs for example.
			//
...
//
// It returns true if codebase should exit with 1.
// e.g. At least one annotation result is in diff.
func reportResults(w io.Writer, filteredResultSet *codebase.FilteredResultMap) bool {
...
[2]: https://help.github.com/en/actions/automating-your-workflow-with-github-actions/development-tools-for-github-actions#logging-commands`)
	...
	var names []string
	filteredResultSet.Range(func(name string, results *codebase.FilteredResult) {
	...
	for _, name := range names {
		...
		if err != nil {
			// Should not happen.
			log.Printf("codebase: result not found for %q", name)
			continue
		}
		fmt.Fprintf(w, "codebase: Reporting results for %q\n", name)
		...
		for _, result := range results.FilteredDiagnostic {
			...
			foundNumOverall++
			// If it's not running in GitHub Actions, codebase should exit with 1
		...
		if !foundResultPerName {
			fmt.Fprintf(w, "codebase: No results found for %q. %d results found outside diff.\n", name, filteredNum)
		}
	}
	return shouldFail
}