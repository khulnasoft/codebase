...
import (
	...
	"github.com/khulnasoft/codebase/doghouse"
	"github.com/khulnasoft/codebase/doghouse/client"
...
func TestNewDoghouseCli_returnErrorForGitHubClient(t *testing.T) {
	cleanup := setupEnvs(map[string]string{
		"CODEBASE_TOKEN":            "",
		"GITHUB_ACTIONS":             "true",
		"CODEBASE_GITHUB_API_TOKEN": "", // missing
	})
	defer cleanup()
	if _, err := newDoghouseCli(context.Background()); err == nil {
		t.Error("got no error but want CODEBASE_GITHUB_API_TOKEN missing error")
	}
}

func TestNewDoghouseCli_returnDogHouseClientWithCodebaseToken(t *testing.T) {
	cleanup := setupEnvs(map[string]string{
		"CODEBASE_TOKEN":            "xxx",
		"GITHUB_ACTIONS":             "true",
		"CODEBASE_GITHUB_API_TOKEN": "xxx",
...
func TestDiagnosticResultSet_Project(t *testing.T) {
	defer func(f func(ctx context.Context, conf *project.Config, runners map[string]bool, level string, tee bool) (*codebase.ResultMap, error)) {
		projectRunAndParse = f
	}(projectRunAndParse)

	var wantDiagnosticResult codebase.ResultMap
	wantDiagnosticResult.Store("name1", &codebase.Result{Diagnostics: []*rdf.Diagnostic{
		{
	...
	projectRunAndParse = func(ctx context.Context, conf *project.Config, runners map[string]bool, level string, tee bool) (*codebase.ResultMap, error) {
	...
	got.Range(func(k string, r *codebase.Result) {
...
func TestDiagnosticResultSet_NonProject(t *testing.T) {
	...
	input := `codebase.go:14:14: test message`
	got, err := checkResultSet(context.Background(), strings.NewReader(input), opt, false)
	if err != nil {
		t.Fatal(err)
	}
	var want codebase.ResultMap
	want.Store("golint", &codebase.Result{Diagnostics: []*rdf.Diagnostic{
		{
			Location: &rdf.Location{
				Range: &rdf.Range{Start: &rdf.Position{
					Line:   14,
...
func TestPostResultSet_withReportURL(t *testing.T) {
			...
			if diff := cmp.Diff(req.Annotations, []*doghouse.Annotation{
				{
					Diagnostic: &rdf.Diagnostic{
						...
						Location: &rdf.Location{
							Path: "cmd/khulnasoft/codebase.go",
							Range: &rdf.Range{
	...
	// It assumes the current dir is ./cmd/codebase/
	var resultSet codebase.ResultMap
	resultSet.Store("name1", &codebase.Result{Diagnostics: []*rdf.Diagnostic{
		{
			Location: &rdf.Location{
				Range: &rdf.Range{Start: &rdf.Position{
					Line: 14,
				}},
				Path: "codebase.go", // test relative path
			},
			Message:        "name1: test 1",
			OriginalOutput: "L1\nL2",
		},
		{
			Location: &rdf.Location{
				Path: absPath(t, "codebase.go"), // test abs path
			},
			Message: "name1: test 2",
		},
	}})
	resultSet.Store("name2", &codebase.Result{Diagnostics: []*rdf.Diagnostic{
		{
			Location: &rdf.Location{
				Range: &rdf.Range{Start: &rdf.Position{
					Line: 14,
...
func TestReportResults(t *testing.T) {
	...
	defer cleanup()
	filteredResultSet := new(codebase.FilteredResultMap)
	filteredResultSet.Store("name1", &codebase.FilteredResult{
	...
	filteredResultSet.Store("name2", &codebase.FilteredResult{
		FilteredDiagnostic: []*filter.FilteredDiagnostic{
			{
				Diagnostic: &rdf.Diagnostic{
					OriginalOutput: "name1-L1\nname1-L2",
	...
	want := `codebase: Reporting results for "name1"
name1-L1
name1-L2
codebase: Reporting results for "name2"
codebase: No results found for "name2". 1 results found outside diff.
`
...
func TestReportResults_noResultsShouldReport(t *testing.T) {
	...
	want := `codebase: Reporting results for "name1"
codebase: No results found for "name1". 2 results found outside diff.
codebase: Reporting results for "name2"
codebase: No results found for "name2". 1 results found outside diff.
`
	if got := stdout.String(); got != want {
