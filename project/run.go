...
func RunAndParse(ctx context.Context, conf *Config, runners map[string]bool, defaultLevel string, teeMode bool) (*codebase.ResultMap, error) {
	var results codebase.ResultMap
	...
	for key, runner := range conf.Runner {
		...
		semaphore <- 1
		log.Printf("codebase: [start]\trunner=%s", runnerName)
		fname := runner.Format
		...
		g.Go(func() error {
			...
			cmdErr := cmd.Wait()
			results.Store(runnerName, &codebase.Result{
				Name:        runnerName,
				Level:       level,
				Diagnostics: diagnostics,
				CmdErr:      cmdErr,
			})
			msg := fmt.Sprintf("codebase: [finish]\trunner=%s", runnerName)
			if cmdErr != nil {
	...
	if err := g.Wait(); err != nil {
		return nil, fmt.Errorf("fail to run codebase: %w", err)
	}
...
// Run runs codebase tasks based on Config.
func Run(ctx context.Context, conf *Config, runners map[string]bool, c codebase.CommentService, d codebase.DiffService, teeMode bool, filterMode filter.Mode, failOnError bool) error {
	...
	var g errgroup.Group
	results.Range(func(toolname string, result *codebase.Result) {
		ds := result.Diagnostics
		g.Go(func() error {
			if err := result.CheckUnexpectedFailure(); err != nil {
				return err
			}
			return codebase.RunFromResult(ctx, c, ds, filediffs, d.Strip(), toolname, filterMode, failOnError)
		})
	})
	return g.Wait()
}
