...
type fakeCommentService struct {
	codebase.CommentService
	FakePost func(*codebase.Comment) error
}

func (f *fakeCommentService) Post(_ context.Context, c *codebase.Comment) error {
	return f.FakePost(c)
}
...
func TestRun(t *testing.T) {
	...
	t.Run("cmd error with findings (not for codebase to exit with error)", func(t *testing.T) {
		...
		cs := &fakeCommentService{
			FakePost: func(c *codebase.Comment) error {
				return nil
			},
		}
		conf := &Config{
	...
	t.Run("unexpected cmd error (codebase exits with error)", func(t *testing.T) {
		buf := new(bytes.Buffer)
		defaultTeeStderr = buf
		ds := &fakeDiffService{
			FakeDiff: func() ([]byte, error) {
